...
#define FW_IMAGE_NAME "synaptics/startup_fw_update_lansi.img"
...
int get_tddi_lockdown_data_lansi(unsigned char *lockdown_data, unsigned short leng)
{
	int retval;
...
int set_tddi_lockdown_data_lansi(unsigned char *lockdown_data, unsigned short leng)
{
	int retval = -EINVAL;
	unsigned long checksum;
	unsigned char checksum_array[4];
...
static int fwu_do_lockdown_v5v6(void)
{
	...
	if (fwu->has_lockdown_data) {
		retval = set_tddi_lockdown_data_lansi(img_ld,
				LOCKDOWN_SIZE);
		if (retval < 0)
			dev_err(rmi4_data->pdev->dev.parent,
...
EXPORT_SYMBOL(synaptics_fw_updater_lansi);
...
static void fwu_startup_fw_update_work(struct work_struct *work)
{
	...
	synaptics_fw_updater_lansi(NULL);
...
exit:
	mutex_unlock(&fwu_sysfs_mutex_lansi);
	return retval;
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	retval = synaptics_fw_updater_lansi(fwu->image);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to do reflash\n",
				__func__);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (get_tddi_lockdown_data_lansi(lockdown_data, lockdown_data_size) < 0) {
		kfree(lockdown_data);
		mutex_unlock(&fwu_sysfs_mutex_lansi);
		return -EINVAL;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (set_tddi_lockdown_data_lansi(lockdown_data, lockdown_data_size) < 0) {
		kfree(lockdown_data);
		mutex_unlock(&fwu_sysfs_mutex_lansi);
		return -EINVAL;
	}
...
exit:
	complete(&fwu_remove_complete_lansi);
...
{
	synaptics_rmi4_new_function_lansi(&fwu_module, true);
...
{
	synaptics_rmi4_new_function_lansi(&fwu_module, false);

	wait_for_completion(&fwu_remove_complete_lansi);
